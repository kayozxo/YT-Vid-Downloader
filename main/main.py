    ...
    try:
        ...
        if save_path:
            ...
            finishLabel.configure(
                text=f"Selected Folder: {save_path}", text_color="white"
            )
            DS.download(output_path=save_path)
            CTkMessagebox(
                title="Success",
                message="Video is successfully downloaded.",
                icon="check",
            )
...
title = customtkinter.CTkLabel(
    app, text="YT-VIDEO DOWNLOADER", font=("Montserrat Black", 20)
)
...
res_selection = customtkinter.CTkComboBox(
    ...
    height=34,
    font=("Montserrat Medium", 12),
    dropdown_font=("Montserrat Medium", 12),
    corner_radius=5,
    state="readonly",
...
# thumbnail download
td = customtkinter.CTkButton(
    app,
    text="Download Image",
    font=("Montserrat Medium", 12),
    command=download_thumbnail,
    corner_radius=5,
    width=140,
    height=34,
)
